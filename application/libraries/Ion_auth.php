...
class Ion_auth
{
	...
	public function driver_forgotten_password($identity)
	{
		if ($this->ion_auth_model->driver_forgotten_password($identity))
		{
			// Get user information
			$identifier = $this->ion_auth_model->identity_column; // use model identity column, so it can be overridden in a controller
			$driver = $this->db->select('*')
				->where('email',$identity)
				->where('active',1)
				->get($this->tables['drivers'])->row();

			if ($driver)
			{
				$data = array(
					'identity' => $driver->{$this->config->item('identity', 'ion_auth')},
					'forgotten_password_code' => $driver->forgotten_password_code
				);
	...
	public function restaurant_forgotten_password($identity)
	{
		...
		{
			// Generate code
			$code = $this->ion_auth_model->restaurant_forgotten_password($identity);
	...
	public function sub_admin_forgotten_password($identity)
	{
		// Retrieve user information
		$user = $this->db->select('*')
				->where('email',$identity)
		...
		{
			// Generate code
			$code = $this->ion_auth_model->sub_admin_forgotten_password($identity);
	 ...
	 * @author Michael
	 */
	public function driver_forgotten_password_check($code)
	{
		$profile = $this->ion_auth_model->get_driver_forgotten_password_code($code); // pass the code to profile
		...
		else
		{
			if ($this->config->item('forgot_password_expiration', 'ion_auth') > 0)
			{
				...
				{
					//it has expired
					$this->ion_auth_model->clear_driver_forgotten_password_code($code);
					$this->set_error('password_change_unsuccessful');
					return FALSE;
				}
			}
	...
	public function admin_forgotten_password_check($code)
	{
		$user = $this->ion_auth_model->get_admin_forgotten_password_code($code);

		if (!is_object($user))
		{
			$this->set_error('Password has been already changed');
			return FALSE;
		}
		else
		{
			if ($this->config->item('forgot_password_expiration', 'ion_auth') > 0)
			{
				...
				if (time() - $user->forgotten_password_time > $expiration)
				{
					...
					$this->set_error('forgot_password_link_expired');
					return FALSE;
				}
			}
			return $user;
	...
	public function sub_admin_forgotten_password_check($code)
	{
		$user = $this->ion_auth_model->get_sub_admin_forgotten_password_code($code);
		...
		else
		{
			if ($this->config->item('forgot_password_expiration', 'ion_auth') > 0)
			{
				...
				if (time() - $user->forgotten_password_time > $expiration)
				{
					//it has expired
					$identity = $user->{$this->config->item('identity', 'ion_auth')};
					$this->ion_auth_model->clear_forgotten_password_code_sub_admin($identity);
					$this->set_error('forgot_password_link_expired');
					return FALSE;
				}
			}
