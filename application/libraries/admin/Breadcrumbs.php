	...
	function show()
	{
		if ($this->breadcrumbs)
        {
			...
			foreach ($this->breadcrumbs as $key => $value)
            {
                ...
                if (reset($keys) == $key)
                {
					$output.= "\t\t\t". $this->breadcrumb_el_open .'<a href="'. $value['href'] .'">'. $this->breadcrumb_el_first .' '. $value['page'] .' <i class="zmdi zmdi-chevron-right"></i></a> '. $this->breadcrumb_el_close ."\n";
				}
                elseif (end($keys) == $key)
                {
                ...
                {
					$output.= "\t\t\t". $this->breadcrumb_el_open .'<a href="'. $value['href'] .'">'. $value['page'] .' <i class="zmdi zmdi-chevron-right"></i></a> '. $this->breadcrumb_el_close ."\n";
				}
            }
