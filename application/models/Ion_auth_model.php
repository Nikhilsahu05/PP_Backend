...
class Ion_auth_model extends CI_Model
{
	...
	public function identity_check_driver($identity = '')
	{
		$this->trigger_events('identity_check');
		...
		return $this->db->where($this->identity_column, $identity)
						->limit(1)
						->count_all_results($this->tables['drivers']) > 0;
	}
	...
	public function login_social($social_id) {
        ...
        $query = $this->db->select('id,activation_code, active, email, id, password, active, last_login, first_name, last_name,username,profile_picture,first_time,phone,is_verified_phone,first_time,unique_id,country_code,is_deleted')
	 ...
	 * @author Ben Edmunds
	 */
	public function update_last_login_driver($id)
	{
		$this->trigger_events('update_last_login');
		...
		$this->db->update($this->tables['drivers'], ['last_login' => time()], ['id' => $id]);
	...
	/* User Login */
	public function user_login_api($identity, $password, $remember = FALSE) {
		$this->trigger_events('pre_login');

		if (empty($identity) || empty($password)) {
			return array('status' => 0,'message' => $this->lang->line('login_empty_parameter'));
		}

		$this->trigger_events('extra_where');

		$query = $this->db->select($this->identity_column . ', users.email, users.password,users.id,users.active, users.first_name,users.last_name,users.phone,users.is_verified_phone,users.unique_id,users.country_code,users.is_deleted,users.is_suspended, users.profile_picture')
		->from('users')
        ->where($this->identity_column, $identity) //$this->identity_column
        ->limit(1)
        ->order_by('id', 'desc')
        ->get();
        ...
        if ($this->is_max_login_attempts_exceeded($identity)) {
        	...
        	return array('status' => 0,'message' => $this->lang->line('login_timeout'));
        }
        ...
        if ($query->num_rows() === 1) {
        	...
        	if ($password === TRUE) {
        		if ($user->active == 0) {
        			...
        			$response = array('status' => 2, 'message' => $this->lang->line('login_unsuccessful_not_active'),'data' => $user);
        			return $response;
        		}

        		/* Check Account has been deleted or not */
    			if($user->is_deleted == 1){
    				$response = array('status' => 0, 'message' => $this->lang->line('login_user_account_deleted'),'data' => $user);
    				return $response;
    			}

    			/* Check Account has been suspended or not */
    			if($user->is_suspended == 1){
    				$response = array('status' => 0, 'message' => $this->lang->line('login_user_account_suspended'),'data' => $user);
    				return $response;
    			}
        		...
        		$response = array('status' => 1, 'message' => $this->lang->line('login_successful'), 'data' => $user);
        		return $response;
        	}

        	$response = array('status' => 0, 'message' => $this->lang->line('login_unsuccessful'));
        	return $response;
        }
        ...
        $this->trigger_events('post_login_unsuccessful');
        $this->set_error('login_unsuccessful');
        $response = array('status' => 0, 'message' => $this->lang->line('login_account_not_found'));
        return $response;
    }

	public function phone_login_api($phone, $remember = FALSE, $code = false) {
		$this->trigger_events('pre_login');
		...
		$query = $this->db->select($this->identity_column . ', users.email, users.password,users.id,users.active, users.first_name,users.last_name,users.phone,users.is_verified_phone,users.unique_id,users.country_code,users.is_deleted,users.is_suspended, users.profile_picture, users.first_time,users.otp')
		->from('users')
        ->where('users.phone', $phone) //$this->identity_column
        ->limit(1)
        ->order_by('id', 'desc')
        ->get();
        ...
        if ($query->num_rows() === 1) {
        	$user = $query->row();
        		...
        		$this->db->update('users',$update_arr,['id' =>$user->id]);
        		send_sms(array('mobile_nuber'=>$user->phone,'message'=>$message,'template_id'=>'1207167508255462029'));
