...
class Party_model extends CI_Model {
    ...
    private function _get_datatables_query() {
        ...
        $this->db->from($this->table);
            ...
            $this->db->where($this->table . '.approval_status','1');
        }else if(!empty($this->input->post('Status')) && $this->input->post('Status')==2) {
            $this->db->where($this->table . '.approval_status','0');
        }
    ...
    public function delete($id) {
      ...
      public function get_status($party_id) {
        return $this->db->get_where($this->table, ['id' => $party_id])->row();
    }

     public function count_organization_all() {
        //$this->db->where($this->table.'.is_deleted', 0);
        //$this->db->where($this->table.'.active', 1);
        $this->db->from($this->table_organization);
        $query = $this->db->get();
        return $query->num_rows();
    }

     public function count_organization_verified_all() {
        $this->db->from($this->table_organization);
        $this->db->where($this->table_organization . '.status',1);
        return $this->db->count_all_results();
    }

      public function count_organization_unverified_all() {
        $this->db->from($this->table_organization);
        $this->db->where($this->table_organization . '.status',0);
        return $this->db->count_all_results();
    }
